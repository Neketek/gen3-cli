...
def parse_stack_filename(filename):
    ...
    pattern = r'([a-z0-9]+)-(.+)-([1-9][0-9]{10})-([a-z]{2}-[a-z]+-[1-9])(-pseudo)?-(.+)'
...
def upgrade_build_ref(root_dir, dry_run):
    ...
    for legacy_file in legacy_files:
        ...
        try:
            build_data = {
                'Commit': build_array[0]
            }
        except IndexError as e:
            raise Exception(f"Unable to upgrade build reference in {legacy_file}") from e
        try:
...
def upgrade_credentials(root_dir, dry_run):
    legacy_files = Search.match_files(os.path.join('**', 'credentials.json'), root=root_dir)
    for legacy_file in legacy_files:
        logger.debug('Checking %s', legacy_file)
        with open(legacy_file, 'rt') as f:
    ...
    for source in UPGRADE_V1_1_0_SOURCES:
        ...
        for source_dir in source_dirs:
            ...
            if source == 'solutions':
                ...
                segment_files = Search.match_files(os.path.join('**', 'segment.json'), root=target_dir)
                for segment_file in segment_files:
                ...
                pem_files = Search.match_files(os.path.join('**', 'aws-ssh*.pem'), root=target_dir)
...
def upgrade_cmdb_repo_to_v1_2_0(root_dir, dry_run):
    legacy_files = Search.match_files(os.path.join('**', 'container_*.ftl'), root=root_dir)
    for legacy_file in legacy_files:
        logger.debug('Checking %s', legacy_file)
        replacement_filename = os.path.basename(legacy_file).replace('container_', 'fragment_')
        replacement_file = os.path.join(os.path.dirname(legacy_file), replacement_filename)
...
def upgrade_cmdb_repo_to_v1_3_0(root_dir, dry_run):
    # Find accounts
    account_files = Search.match_files(os.path.join('**', 'account.json'), root=root_dir)
    ...
    for cf_dir in cf_dirs:
        cmk_stacks = Search.match_files(os.path.join('**', 'seg-cmk-*[0-9]-stack.json'), root=cf_dir)
        for cmk_stack in cmk_stacks:
            ...
            if cmk_account:
                ...
                segment_stacks = Search.match_files(os.path.join('**', '*stack.json'), root=cmk_path)
                ...
                pem_files = Search.match_files(os.path.join('**', '.aws-ssh*.pem*'), root=operations_path)
                for pem_file in pem_files:
                    pem_dir = os.path.dirname(pem_file)
                    pem_basename = os.path.basename(pem_file)
                    new_basename = pem_basename.replace('aws-', f'aws-{cmk_account_id}-{cmk_region}-')
                    # Move the pem files to make them invisible to the generation process
                    src = pem_file
...
def upgrade_cmdb_repo_to_v2_0_0(root_dir, dry_run):
    ...
    for config_dir in config_dirs:
        ...
        build_files = [filename for filename in build_files if not filename.endswith('build.json')]
...
def upgrade_cmdb_repo_to_v2_0_1(root_dir, dry_run):
    ...
    for state_dir in state_dirs:
        ...
        for df_dir in deployment_frameworks:
            ...
            logger.debug('%sChecking %s deployment_framework', dry_run, deployment_framework)
            ...
            for state_file in state_files:
                ...
                if not stack_deployment_unit:
                    ...
                    match = re.match(r"account-([a-z][a-z0-9]+)-([a-z]{2}-[a-z]+-[1-9])-(.+)", state_basename)
                    if match:
                        stack_level = "account"
                        stack_deployment_unit = match.group(1)
                        stack_region = match.group(2)
                    match = re.match(r"account-([a-z]{2}-[a-z]+-[1-9])-(.+)", state_basename)
                    if match:
                        stack_level = "account"
                ...
                if not stack_deployment_unit:
                    ...
                    match = re.match(r"product-([a-z]{2}-[a-z]+-[1-9])-(.+)", state_basename)
                    if match:
                        stack_level = "product"
                ...
                if not stack_deployment_unit:
                    ...
                    match = re.match(r"seg-key-([a-z]{2}-[a-z]+-[1-9])-(.+)", state_basename)
                    if match:
                        stack_level = "seg"
                        stack_deployment_unit = "cmk"
                        stack_region = match.group(1)
                    match = re.match(r"cont-([a-z][a-z0-9]+)-([a-z]{2}-[a-z]+-[1-9])-(.+)", state_basename)
                    if match:
                        stack_level = "seg"
                        stack_deployment_unit = match.group(1)
                        stack_region = match.group(2)
...
def process_cmdb(
    ...
    for cmdb_git_repo in cmdb_git_repos:
        ...
        if os.path.isfile(cmdb_version_file):
            ...
            current_version = cmdb_version_data.get('Version', {}).get(action.capitalize())
        ...
        for version in versions:
            if utils.semver_compare(current_version, version) >= 0:
                logger.debug('%s of repo "%s" to %s is not required', action.capitalize(), cmdb_repo, version)
            else:
                logger.info('%s%s of repo "%s" to %s required ...', dry_run, action.capitalize(), cmdb_repo, version)
            if pin_version:
                if utils.semver_compare(current_version, pin_version) < 0:
                    ...
                    logger.debug('%s%s of repo "%s" to %s permitted by pin version %s', dry_run, action.capitalize())
